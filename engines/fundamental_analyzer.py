...
def analyze_fundamentals(ticker):
    ...
    frames = get_financial_frames(ticker)
    ...
    if debt_to_equity is None and latest_debt and latest_equity:
        debt_to_equity = (latest_debt / latest_equity) * 100
    ...
    metrics = {
        ...
        "roe": _fmt_pct((info.get("returnOnEquity") or 0) * 100) if info.get("returnOnEquity") else None,
        "roce": None,
        ...
        "pe_ratio": _fmt_pct(info.get("trailingPE")),
        "pb_ratio": _fmt_pct(info.get("priceToBook")),
        "current_ratio": _fmt_pct(current_ratio),
        "quarterly_revenue_growth": _fmt_pct(info.get("revenueGrowth") * 100) if info.get("revenueGrowth") else _fmt_pct(_growth(_first_available(quarterly, ["Total Revenue"]))),
        "institutional_holding": _fmt_pct(info.get("heldPercentInstitutions") * 100) if info.get("heldPercentInstitutions") else None,
    ...
    checks = [
        ...
        _score_metric("Debt to equity", metrics["debt_to_equity"], 60, 120, inverse=True),
    ...
    normalized_score = int(max(0, min(100, 50 + score * 5)))
    ...
    return {
        ...
        "data_quality": "Good" if annual is not None and not annual.empty else "Limited"
    }
